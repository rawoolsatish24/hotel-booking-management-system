<?php	
	...
	function send_email($receiver_email, $subject, $body)
	{
		try {
			...
			$mail->IsHTML(true);
			$mail->CharSet = 'UTF-8';
			$mail->Username = "HotelBooking@gmail.com";
			$mail->Password = "HotelBookingPassword";
			$mail->From = 'HotelBooking@gmail.com';
			$mail->FromName = 'Help@HBMS';
	...
	function updateStatusToUser($bookingid) {
		try {
			...
			if(!empty($bookingDetails)) {
				$subject = 'Room Status Updated - HBMS!';
				$body = '
					<center><h2>Room Status Updated - HBMS!</h2></center>
					<p>Hi '.getManneredUserName($bookingDetails['first_name'], $bookingDetails['last_name']).',</p>
					<p>The room status have been updated as below mentioned, please check for further process. Also, you can track your booking status by visiting <a href="http://localhost/HotelBookingsManagementSystem/index.php" target="_blank"><b>HBMS</b></a> this link.</p>
